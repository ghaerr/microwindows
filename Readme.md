...
on Linux, OSX, Android and other platforms. It is
Open Source and licenced under the the Mozilla Public License.
The main GUI toolkit that can be used with the Nano-X API is FLTK, while
the Win32 API builtin controls are included.
...
are the Nano-X API and the Win32 API which can be used to write applications.
To provide close X11 compatibility there is the NX11 library
which is built on top of the Nano-X API, which allows X11 applications to be linked
without recompilation.  The FLTK GUI toolkit runs based on NX11.

## Areas where Microwindows can be used are:

- provide an excellent GUI for small embedded devices with FLTK or Win32
...
Raspberry Pi, DOS DJGPP, RTEMS, Sony PSP, Nintendo DS** and more. 
