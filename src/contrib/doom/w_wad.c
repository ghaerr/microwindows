...
#if defined(linux) || defined(__BEOS__) || defined(__SVR4) || ALWAYS
void strupr (char* s)
{
...
//printf("# lumps %d, length %d\n", header.numlumps, length);
    ...
    storehandle = reloadname ? -1L : handle;
...
//printf("lump %d %.8s offset %d size %d\n", i, fileinfo->name, fileinfo->filepos, fileinfo->size);
    }
...
//printf("lump %d fseek %lx, %ld size %d\n", lump, handle, l->position, l->size);
