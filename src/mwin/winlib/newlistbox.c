...
/* combo state struct (WINE controls.h) */
typedef struct {
   HWND           self;
   HWND           owner;
   UINT           dwStyle;
...
static void LISTBOX_PaintItem( HWND hwnd, LB_DESCR *descr, HDC hdc,
                               RECT *rect, INT index, UINT action, BOOL ignoreFocus )
{
...
//	        FIXME("called with an out of bounds index %d(%d) in owner draw, Not good.\n",index,descr->nb_items);
        ...
        dis.rcItem       = *rect;
        SendMessage(descr->owner, WM_DRAWITEM, id, (LPARAM)&dis);
    }
    else
    {
        COLORREF oldText = 0, oldBk = 0;
#if 0 /* No ScrollWindowEx yet */
        int hOffset = 0;
#else
        int hOffset = descr->horz_pos;
#endif
        ...
        if (!item)
            ExtTextOut( hdc, rect->left + 1 - hOffset, rect->top,
        ...
        else
	{
	    /* Output empty string to paint background in the full width. */
            ExtTextOut( hdc, rect->left + 1 - hOffset, rect->top,
                         ETO_OPAQUE | ETO_CLIPPED, rect, NULL, 0, NULL );
            TabbedTextOut( hdc, rect->left + 1 - hOffset , rect->top,
                            item->str, -1,
                            descr->nb_tabs, descr->tabs, -hOffset);
	}
        if (item && item->selected)
        {
                                 ...
                                 UINT action )
{
    ...
    hbrush = (HBRUSH)(LRESULT)SendMessage( descr->owner, WM_CTLCOLORLISTBOX,
				   (WPARAM)hdc, (LPARAM)hwnd );
    if (hbrush) oldBrush = SelectObject( hdc, hbrush );
    if (!IsWindowEnabled(hwnd))
        SetTextColor( hdc, GetSysColor( COLOR_GRAYTEXT ) );
#if 0 /* no SetWindowOrgEx */
...
static LRESULT LISTBOX_InitStorage( HWND hwnd, LB_DESCR *descr, INT nb_items )
{
    ...
    if (descr->items) nb_items++;
    if (!(item = realloc ( descr->items, nb_items * sizeof(LB_ITEMDATA) )))
    {
 ...
 *           LISTBOX_GetText
 */
static LRESULT LISTBOX_GetText( LB_DESCR *descr, INT index, LPARAM lParam)
{
    if ((index < 0) || (index >= descr->nb_items)) return LB_ERR;
    if (HAS_STRINGS(descr))
    {
    	LPCSTR buffer = (LPCSTR)lParam;
...
static INT LISTBOX_FindStringPos( HWND hwnd, LB_DESCR *descr, LPCTSTR str, BOOL exact )
{
    ...
    while (min != max)
    {
        index = (min + max) / 2;
        if (HAS_STRINGS(descr))
            res = strcasecmp( str, descr->items[index].str);
        else
        {
            COMPAREITEMSTRUCT cis = {0};
            UINT id = GetWindowLong( hwnd, GWL_ID );
...
#endif
            res = SendMessage( descr->owner, WM_COMPAREITEM, id, (LPARAM)&cis );
        }
        if (!res) return index;
        if (res < 0) max = index;
        else min = index + 1;
 ...
 */
static INT LISTBOX_FindFileStrPos( HWND hwnd, LB_DESCR *descr, LPCTSTR str )
{
    INT min, max, res = -1;
    ...
    {
        INT index = (min + max) / 2;
        LPCTSTR p = descr->items[index].str;
    ...
    if (HAS_STRINGS(descr))
    {
        ...
        if (exact)
        {
            for (i = start + 1; i < descr->nb_items; i++, item++)
                if (!strcasecmp( str, item->str )) return i;
        ...
        if ((descr->style & LBS_MULTICOLUMN) && !col_pos)
        {
            if (!IS_OWNERDRAW(descr))
            {
                ...
                if (rect.top < descr->height)
                {
                    ...
                    ExtTextOut( hdc, 0, 0, ETO_OPAQUE | ETO_CLIPPED,
                                   &rect, NULL, 0, NULL );
                }
            }
                                   ...
                                   LPCSTR str, ULONG_PTR data )
{
    ...
    if (descr->nb_items == max_items)
    {
		...
		if (descr->items&&!(item = realloc ( descr->items, max_items * sizeof(LB_ITEMDATA) )))
        {
            SEND_NOTIFICATION( hwnd, descr, LBN_ERRSPACE );
            return LB_ERRSPACE;
        }
		else if((!descr->items)&&!(item = malloc( max_items * sizeof(LB_ITEMDATA) )))
        {
            SEND_NOTIFICATION( hwnd, descr, LBN_ERRSPACE );
            return LB_ERRSPACE;
        }
    ...
    if (descr->style & LBS_OWNERDRAWVARIABLE)
    {
        ...
        SendMessage( descr->owner, WM_MEASUREITEM, id, (LPARAM)&mis );
...
static LRESULT LISTBOX_RemoveItem( HWND hwnd, LB_DESCR *descr, INT index )
{
    ...
    {
        max_items -= LB_ARRAY_GRANULARITY;
        item = realloc ( descr->items, max_items * sizeof(LB_ITEMDATA) );
...
static void LISTBOX_ResetContent( HWND hwnd, LB_DESCR *descr )
{
    ...
    if (descr->items) free( descr->items );
    descr->nb_items      = 0;
    descr->top_item      = 0;
    descr->selected_item = -1;
    descr->focus_item    = 0;
...
static LRESULT LISTBOX_HandleHScroll( HWND hwnd, LB_DESCR *descr, WPARAM wParam )
{
    ...
    else if (descr->horz_extent)
    {
        ...
        {
        case SB_LINELEFT:
            LISTBOX_SetHorizontalPos( hwnd, descr, descr->horz_pos - 10 );
            break;
        case SB_LINERIGHT:
            LISTBOX_SetHorizontalPos( hwnd, descr, descr->horz_pos + 10 );
            break;
                                          ...
                                          WPARAM wParam, INT x, INT y )
{
    ...
    if (!descr->lphc)
    {
        if (descr->style & LBS_NOTIFY )
            SendMessage( descr->owner, WM_LBTRACKPOINT, index,
                            MAKELPARAM( x, y ) );
#if 0 /* No Drag&Drop */
        if (GetWindowLong( hwnd, GWL_EXSTYLE ) & WS_EX_DRAGDETECT)
        {
            POINT pt;
	    ...
	    pt.y = y;
            ...
            if (DragDetect( hwnd, pt ))
                SendMessage( descr->owner, WM_BEGINDRAG, 0, 0 );
        }
#endif
    }
    return 0;
                                               ...
                                               UINT msg, WPARAM wParam, INT x, INT y)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if(nHitTestType != 0)
            {
                SendMessage(hwnd, WM_NCLBUTTONDOWN, nHitTestType,
...
static LRESULT LISTBOX_HandleKeyDown( HWND hwnd, LB_DESCR *descr, WPARAM wParam )
{
    ...
    {
        caret = SendMessage( descr->owner, WM_VKEYTOITEM,
 ...
 *           LISTBOX_HandleChar
 */
static LRESULT LISTBOX_HandleChar( HWND hwnd, LB_DESCR *descr, char charA )
{
    INT caret = -1;
    char str[2];
    ...
    if (descr->style & LBS_WANTKEYBOARDINPUT)
    {
        caret = SendMessage( descr->owner, WM_CHARTOITEM,
                                MAKEWPARAM(charA, descr->focus_item),
                                (LPARAM)hwnd );
        if (caret == -2) return 0;
    }
    if (caret == -1)
...
static BOOL LISTBOX_Create( HWND hwnd, LPHEADCOMBO lphc )
{
    ...
    if (!(descr = malloc ( sizeof(*descr) )))
        return FALSE;

    GetClientRect( hwnd, &rect );
    descr->owner         = GetParent( hwnd );
    descr->style         = GetWindowLong( hwnd, GWL_STYLE );
    descr->width         = rect.right - rect.left;
    descr->height        = rect.bottom - rect.top;
    descr->items         = NULL;
    descr->nb_items      = 0;
...
static LRESULT CALLBACK ListBoxWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (!(descr = (LB_DESCR *)(ULONG_PTR)GetWindowLongPtr( hwnd, 0 )))
    {
        if (msg == WM_CREATE)
        {
  ...
  TRACE("[%p]: msg %s wp %08x lp %08lx\n",
    ...
    case LB_ADDSTRING:
    {
        ...
        wParam = LISTBOX_FindStringPos( hwnd, descr, text, FALSE );
        ret = LISTBOX_InsertString( hwnd, descr, wParam, text );
        return ret;
    }
    ...
    case LB_GETTEXT:
        return LISTBOX_GetText( descr, wParam, lParam);
    ...
    case LB_FINDSTRING:
    {
        INT ret;
        LPCSTR text = (LPCSTR)lParam;
        ret = LISTBOX_FindString( hwnd, descr, wParam, text, FALSE );
        return ret;
    }
    ...
    case LB_FINDSTRINGEXACT:
    {
        INT ret;
        LPCSTR text  = (LPCSTR)lParam;
        ret = LISTBOX_FindString( hwnd, descr, wParam, text, TRUE );
        return ret;
    }
    ...
    case LB_SELECTSTRING:
    {
        ...
        index = LISTBOX_FindString( hwnd, descr, wParam, text, FALSE );
        if (index != LB_ERR)
	{
    ...
    case LB_SETTABSTOPS:
        return LISTBOX_SetTabStops( hwnd, descr, wParam, (LPINT)lParam );
    ...
    case WM_CHAR:
        return LISTBOX_HandleChar( hwnd, descr, (char)wParam );
#if 0
    case WM_SYSTIMER:
    ...
    case WM_ERASEBKGND:
	{
		...
		if ((hbr = (HBRUSH)(LRESULT)SendMessage (descr->owner, WM_CTLCOLORLISTBOX, wParam, (LPARAM)hwnd)) != 0)
		{
    ...
    case WM_DROPFILES:
        if( !descr->lphc )
            return SendMessage( descr->owner, msg, wParam, lParam );
        break;
#endif
    default:
        return DefWindowProc( hwnd, msg, wParam, lParam );
