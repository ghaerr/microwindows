...
int	DecodeRLE8 ( UCHAR *buf, UCHAR **fp, int linesize );
int	DecodeRLE4 ( UCHAR *buf, UCHAR **fp, int linesize );
void	put4(int b);


/*
 *  Loads a bitmap from resource file.
 */
PMWIMAGEHDR resLoadBitmap ( HINSTANCE hInst, LPCTSTR resName )
{
	...
	do  {
		...
		do  {
			...
			if( bitdepth <= 8 )
				{
				pImageHdr->palette = (MWPALENTRY*) malloc ( 256*sizeof(MWPALENTRY) );
			...
			if( compression == BI_RLE8 )
				{
				for ( i=cy-1; i >= 0; i--, pLine += linesize )
					{
	...
	if( retV == NULL )
		{
		if( pImageHdr->palette ) free ( pImageHdr->palette );
		if( pImageHdr->imagebits ) free ( pImageHdr->imagebits );
		if( pImageHdr ) free ( pImageHdr );
		}
	return retV;
}
 ...
 */
int DecodeRLE8 ( UCHAR *buf, UCHAR **pfp, int linesize )
{
int c, n;
UCHAR *fp = *pfp;
UCHAR *p = buf;
...
int DecodeRLE4 ( UCHAR *buf, UCHAR **pfp, int linesize )
{
int		c, n, c1, c2;
UCHAR *fp = *pfp;
UCHAR *end;
