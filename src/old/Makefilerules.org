...
ifeq ($(ARCH),MACOSX)
X11LDFLAGS += -L/opt/X11/lib -lX11					# OSX X11 libs
else
X11LDFLAGS += -L/usr/X11R6/lib -lX11				# standard linux X11 libs
#X11LDFLAGS += -L/usr/lib/x86_64-linux-gnu -lX11		# ubuntu X11 libs
endif
...
DEFINES += -DX11=1
LDFLAGS += $(X11LDFLAGS)
HOSTCFLAGS += -I/usr/X11R6/include
...
#Harfbuzz together with Freetype only
...
#some screen drivers use this instead of MWPIXEL_FORMAT defined above
ifdef SCREEN_PIXTYPE
DEFINES += -DSCREEN_PIXTYPE=$(SCREEN_PIXTYPE)
HOSTCFLAGS += -DSCREEN_PIXTYPE=$(SCREEN_PIXTYPE)
endif
...
# these parameters may not be used with c++
TEMPVAR = $(patsubst -Wno-pointer-to-int-cast, ,$(CFLAGS))
CXXFLAGS = $(patsubst -Wno-missing-prototypes, ,$(TEMPVAR))
...
#export for nxlib/test makefile - CC and CFLAGS not passed ARCH for TinyWidgets
export NANOXCLIENTLIBS CCNANOXCLIENTLIBS LDFLAGS $(CC) $(CFLAGS) TOOLSPREFIX MW_DIR_LIB SHOWSTUBS OPTFLAGS PARALLEL ARCH DEBUG
##############################################################################
# Build rules
##############################################################################
...
cleanusr:
	...
	-rm -f /usr/lib/libmwengine.so
	-rm -f /usr/lib/libmwimages.a
	-rm -f /usr/lib/libmwimages.so
	...
	-rm -f /usr/lib/libNXext.a
	-rm -f /usr/lib/libNXext.so
	-rm -f /usr/lib/libNXext.so.0	

	-rm -f /usr/lib64/libnano-X.a
	-rm -f /usr/lib64/libnano-X.so

	-rm -f /usr/lib64/libmwengine.a
	-rm -f /usr/lib64/libmwengine.so
	-rm -f /usr/lib64/libmwimages.a
	-rm -f /usr/lib64/libmwimages.so
	...
	-rm -f /usr/lib64/libPX11.a
	-rm -f /usr/lib64/libPX11.so
	-rm -f /usr/lib64/libPX11.so.0	
	-rm -f /usr/lib64/libNXext.a
	-rm -f /usr/lib64/libNXext.so
	-rm -f /usr/lib64/libNXext.so.0	
	#avoid "Cannot bind to named socket" error - see faq.html
	-rm -f /tmp/.nano-X
ifeq ($(TINYWIDGETS), Y)
...
ifeq ($(SHAREDLIBS), Y)
	...
	# copy logical links
	echo "Copying logical links to $(LIBINSTALLDIR)"; 
	-$(CP) -d $(MW_DIR_LIB)/*.so.* $(LIBINSTALLDIR) 2>/dev/null; 
	...
	# copy logical links
	echo "Copying logical links to $(LIBINSTALLDIR)64"; 
	-$(CP) -d $(MW_DIR_LIB)/*.so.* $(LIBINSTALLDIR)64 2>/dev/null; 
else
